...
describe 'Crek trying to parsing an invalid file.' do
  it 'Fail to open a legacy xls file.' do
    lambda { Crek::Book.new 'spec/fixtures/invalid.xls' }.should raise_error 'Not a valid file format.'
  end

  it 'Ignore file extensions on request.' do
    path = 'spec/fixtures/sample-as-zip.zip'
    lambda { Crek::Book.new path, :check_file_extension => false }.should_not raise_error
  end

  it 'Check file extension when requested.' do
    open_book = lambda { Crek::Book.new 'spec/fixtures/invalid.xls', :check_file_extension => true }
  ...
  it 'Check file extension of original_filename if passed.' do
    ...
    lambda { Crek::Book.new path, :original_filename => 'invalid.xls' }.should raise_error 'Not a valid file format.'
    lambda { Crek::Book.new path, :original_filename => 'valid.xlsx' }.should_not raise_error
  end

end

describe 'Crek parsing a sample XLSX file' do
  before(:all) do
    @Crek = Crek::Book.new 'spec/fixtures/sample.xlsx'
  ...
  it 'open an XLSX file successfully.' do
    @Crek.should_not be_nil
  end

  it 'find sheets successfully.' do
    @Crek.sheets.count.should == 1
    sheet = @Crek.sheets.first
  ...
  it 'Parse rows with empty cells successfully.' do
    rows = Array.new
    row_count = 0
    @Crek.sheets[0].rows.each do |row|
      rows << row
      row_count += 1
    end
  ...
  it 'Parse rows with empty cells and meta data successfully.' do
    rows = Array.new
    row_count = 0
    @Crek.sheets[0].rows_with_meta_data.each do |row|
      rows << row
      row_count += 1
    end
